...
import DeleteModule from "@/components/modules/deleteModule.vue";
  ...
  methods:
      {
        openModal() {
          ...
          this.categoryDescription = this.courseId;
          this.showModal = true;
        },
        closeModal() {
        ...
        async submitForm()
        {
          ...
          const responce = await axios.put("/api/CourseModuls/"+this.idProp, formData);
...
<template>
  ...
  <button  @click="openModal"
           ...
           focus:outline-none focus:ring-yellow-300 font-medium rounded-lg
            text-sm p-2 text-center inline-flex items-center dark:bg-yellow-400
             dark:hover:bg-yellow-500 dark:focus:ring-yellow-600">
    <IconEdit></IconEdit>
  </button>
      ...
      <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
        ...
        <div class="px-6 py-6 lg:px-8">
          ...
          <form @submit.prevent="submitForm" class="space-y-6" action="#">
            ...
            <div class="flex ">
              <delete-module :idUser="idProp" />
              <button @click="submitForm"  type="submit"
                      class=" mt-2 w-full text-white bg-yellow-400 hover:bg-yellow-500 focus:ring-4
                       focus:outline-none focus:ring-yellow-300 font-medium rounded-lg text-sm
                        text-center dark:bg-yellow-500 dark:hover:bg-yellow-600 dark:focus:ring-yellow-800">
                {{$t("edit")}}
              </button>
            </div>
