...
export default {
  ...
  methods: {
    ...
    async confirmDelete() {
      ...
      const response = await axios.delete("api/courserequirments/"+this.requirementId);
...
<template>
  <button type="button" @click="openDeleteModal"
          class=" text-white bg-red-700 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300
                     px-1 py-1 rounded-lg text-sm  text-center inline-flex items-center  dark:bg-red-600
